...
import { removeAutoGeneratedZeroInventoryRows } from "./controllers/zoneStockController.js";
...
const startInventorySafeDefaults = async () => {
  try {
    const removedRows = await removeAutoGeneratedZeroInventoryRows();
    if (removedRows) {
      clearResponseCache();
      console.log(`Removed ${removedRows} auto-generated zero inventory row(s).`);
    }
  } catch (error) {
    console.error("Auto-generated inventory cleanup failed:", error.message);
  }

  startMidnightDefaultsScheduler(() => clearResponseCache());
};

startInventorySafeDefaults();
