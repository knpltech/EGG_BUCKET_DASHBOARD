...
/* ADD FOOD ALLOWANCE */
export const addFoodAllowance = async (req, res) => {
  try {
    ...
    const ref = db.collection("foodAllowance").doc(date);
    ...
    if (doc.exists) {
      ...
      if (outlets[outlet] !== undefined) {
        return res.status(400).json({
          error: "Food Allowance already entered for this outlet on this date",
        });
      }
...
/* GET ALL FOOD ALLOWANCES */
export const getAllFoodAllowances = async (req, res) => {
  try {
    const snapshot = await db.collection("foodAllowance").get();
...
export const updateFoodAllowance = async (req, res) => {
  ...
  try {
    await db.collection("foodAllowance").doc(id).update({ outlets, total });
    res.json({ success: true });
  } catch (err) {
...
/* DELETE a specific OUTLET's food allowance for a specific date (admin only) */
export const deleteFoodAllowanceByOutletAndDate = async (req, res) => {
  try {
    ...
    if (!doc.exists) {
      return res.status(404).json({ error: "No food allowance entry found for this date" });
    }
    ...
    res.status(200).json({ message: `Outlet ${outletId} removed from food allowance for ${date}`, count: 1 });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
};

/* DELETE ALL food allowances for a specific date (admin only) */
export const deleteFoodAllowancesByDate = async (req, res) => {
  try {
    ...
    await ref.delete();
    res.status(200).json({ message: "All food allowances deleted for the given date" });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
};
