...
const getPreviousClosingStock = async (zone, dateIso) => {
  ...
  const previous = snap.docs
    ...
    .filter((row) => !isAutomaticInventoryRow(row))
...
export const recalculateZoneStockFromDate = async (zone, startDate) => {
  ...
  const zoneRowsRaw = zoneSnap.docs
    ...
    .filter((row) => row.normalizedDate && !isAutomaticInventoryRow(row));
...
export const ensureMissingZoneStockEntry = async (zone, date) => {
  const normalizedZone = normalizeZoneLabel(zone);
  const normalizedDate = normalizeDate(date);
  if (!normalizedZone || !normalizedDate) return { created: false };

  const existingZoneSnap = await db.collection(COLLECTION).where("zone", "==", normalizedZone).get();
  const alreadySaved = existingZoneSnap.docs.some((doc) => {
    const row = doc.data() || {};
  ...
  const [totals, openingStock, stockOptionsSnap] = await Promise.all([
    ...
    getPreviousClosingStock(normalizedZone, normalizedDate),
    db.collection("stockOptionsEntries").where("zone", "==", normalizedZone).get(),
  ]);

  const stockIn = stockOptionsSnap.docs
    .map((doc) => doc.data() || {})
    .filter((row) => normalizeDate(row.date || row.createdAt) === normalizedDate)
    .reduce((sum, row) => sum + toNumber(row.stockQuantity), 0);
  const closingStock = openingStock + stockIn - totals.salesQty - totals.damagesQty;

  if (closingStock < 0) {
    console.warn(`[inventory midnight] Skipped ${normalizedZone} on ${normalizedDate}: stock would become negative.`);
  ...
  await recalculateZoneStockFromDate(normalizedZone, normalizedDate);
  return { created: true };
};
...
// `autoGenerated` is the persisted field used by the scheduler.  Older rows
// may carry the short-lived `isAutoDefault` name, so treat either one as an
// automatic row while reading.  Keeping this in one predicate prevents an
// automatic row from becoming part of the manual inventory chain.
const isAutomaticInventoryRow = (row) => Boolean(row?.autoGenerated || row?.isAutoDefault);
...
const getEarliestDateByZone = (rows) => {
  ...
  for (const row of rows || []) {
    ...
    if (isAutomaticInventoryRow(row)) continue;
    const zone = normalizeZoneLabel(row?.zone);
    const date = normalizeDate(row?.date || row?.createdAt);
    if (!zone || !date) continue;
...
export const removeAutoGeneratedZeroInventoryRows = async () => {
  const autoStockOptionsSnap = await db.collection("stockOptionsEntries")
    .where("autoGenerated", "==", true)
    .get();

  const autoStockKeys = new Set(autoStockOptionsSnap.docs
    .map((doc) => doc.data() || {})
    .filter((row) => toNumber(row.stockQuantity) === 0)
    .map((row) => {
      const zone = normalizeZoneLabel(row.zone);
      const date = normalizeDate(row.date || row.createdAt);
      return zone && date ? `${zone}__${date}` : "";
    })
    .filter(Boolean));

  if (!autoStockKeys.size) return 0;

  const inventorySnap = await db.collection(COLLECTION).get();
  const rowsToDelete = inventorySnap.docs.filter((doc) => {
    const row = doc.data() || {};
    const zone = normalizeZoneLabel(row.zone);
    const date = normalizeDate(row.date || row.createdAt);
    const key = zone && date ? `${zone}__${date}` : "";
    const hasManualMetadata = Boolean(row.addedBy) || hasNonEmptyRemarks(row);
    return autoStockKeys.has(key) && !row.autoGenerated && !hasManualMetadata && toNumber(row.stockIn) === 0;
  });

  if (!rowsToDelete.length) return 0;

  for (let index = 0; index < rowsToDelete.length; index += 450) {
    const batch = db.batch();
    rowsToDelete.slice(index, index + 450).forEach((doc) => batch.delete(doc.ref));
    await batch.commit();
  }
...
export const upsertZoneStockEntry = async (req, res) => {
  try {
    ...
    if (computedClosingStock < 0) {
      return res.status(409).json({
        message: "Closing stock cannot be negative. Update Stock In before saving.",
...
export const getAllZoneStockEntries = async (_req, res) => {
  try {
    ...
    const data = allRows
      .filter((r) => !isAutomaticInventoryRow(r))
...
export const getZoneStockEntriesByZone = async (req, res) => {
  try {
    ...
    const data = dedupeZoneStockRows(rows).filter((r) => !isAutomaticInventoryRow(r)).sort((a, b) => String(b.date || "").localeCompare(String(a.date || "")));
    return res.status(200).json(data);
  } catch (error) {
    return res.status(500).json({ message: "Error fetching zone stock entries", error: error.message });
  }
...
export const getZoneStockEntriesByZoneAndDate = async (req, res) => {
  try {
    ...
    if (isAutomaticInventoryRow(row)) return res.status(200).json({});
