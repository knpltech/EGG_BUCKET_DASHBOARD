...
export const createStockOptionEntry = async (req, res) => {
  try {
    ...
    const autoEntry = existingDocs.find((row) => row.autoGenerated);
    ...
    const storedPayload = {
      ...payload,
      autoGenerated: false,
      createdAt: autoEntry?.createdAt || new Date(),
    };

    const docRef = autoEntry?.ref || db.collection(COLLECTION).doc();
    await docRef.set(storedPayload, { merge: Boolean(autoEntry) });

    await syncInventoryStockIn(payload.zone, payload.date, updatedQuantity, payload.addedBy);

    return res.status(autoEntry ? 200 : 201).json({
      id: docRef.id,
      ...storedPayload,
...
export const ensureZeroStockOptionEntry = async (zone, date) => {
  const normalizedZone = normalizeZoneLabel(zone);
  const normalizedDate = normalizeDate(date);
  if (!normalizedZone || !normalizedDate) return false;

  const existing = await getStockOptionDocsForZoneDate(normalizedZone, normalizedDate);
