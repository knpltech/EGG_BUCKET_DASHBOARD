...
const getZoneScopedTotal = (entry, zone, zoneOutletKeys) => {
  ...
  return Object.entries(values).reduce((sum, [outletKey, amount]) => {
    ...
    return matchesUserZone || matchesOutlet ? sum + toNumber(amount) : sum;
  }, 0);
};
...
export const ensureCashClosureEntry = async (zone, date) => {
  ...
  const [outletsSnap, cashSnap, incentivesSnap, foodAllowanceSnap, advanceSnap] = await Promise.all([
  ...
  const zoneOutletKeys = getZoneOutletKeys(outletsSnap.docs.map(mapDoc), normalizedZone);
  const firstRow = (snap) => snap.docs[0]?.data() || {};

  await db.collection(COLLECTION).doc(`auto_${normalizedZone.replace(/\s+/g, "_")}_${normalizedDate}`).set({
    zone: normalizedZone,
    date: normalizedDate,
    totalCashAmount: getZoneScopedTotal(firstRow(cashSnap), normalizedZone, zoneOutletKeys),
    incentives: getZoneScopedTotal(firstRow(incentivesSnap), normalizedZone, zoneOutletKeys),
    foodAllowance: getZoneScopedTotal(firstRow(foodAllowanceSnap), normalizedZone, zoneOutletKeys),
    advance: getZoneScopedTotal(firstRow(advanceSnap), normalizedZone, zoneOutletKeys),
    cashHandover: 0,
    cashRemarks: "",
    autoGenerated: true,
    createdAt: new Date(),
