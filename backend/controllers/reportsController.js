...
export const getReports = async (req, res) => {
  ...
  try {
    ...
    const setPreferredValue = (field, dateKey, value, source) => {
      if (value === null || value === undefined) return;
      const entry = ensureDateEntry(dateKey);
      if (!entry) return;

      const numericValue = Number(value) || 0;
      const metadata = valueMetadata[field];
      const existing = metadata.get(dateKey);
      const candidate = {
        value: numericValue,
        manual: !source?.autoGenerated,
        time: getEntryTimeMs(source),
      };
      const shouldReplace = !existing
        || (candidate.manual && !existing.manual)
        || (candidate.value !== 0 && existing.value === 0)
        || (candidate.manual === existing.manual && candidate.time >= existing.time);

      if (shouldReplace) {
        entry[field] = numericValue;
        metadata.set(dateKey, candidate);
      }
    };
    ...
    salesSnapshot.forEach(doc => {
      ...
      setPreferredValue('salesQty', dateKey, salesValue, data);
    });
    ...
    digitalPaymentsSnapshot.forEach(doc => {
      ...
      setPreferredValue('digitalPay', dateKey, digitalValue, data);
    });
...
export const getStatistics = async (req, res) => {
  ...
  try {
    ...
    // Collections may contain an earlier manual/auto document plus the final
    // nightly-sync document for the same outlet and date. Statistics must use
    // one value for that outlet/date/field, never add both documents.
    const preferredMappedValues = new Map();
    const getRecordPriority = (docData) => {
      if (docData.finalSourceSync) return 3;
      if (!docData.autoGenerated) return 2;
      return 1;
    };
    ...
    const addMappedValues = (docData, field, targetField) => {
      ...
      const priority = getRecordPriority(docData);
      const eventTime = getEntryTimeMs(docData);
      ...
      Object.entries(values).forEach(([key, value]) => {
        ...
        const numericValue = Number(value) || 0;
        const candidateKey = `${targetField}__${outlet.id}__${dateKey}`;
        const current = preferredMappedValues.get(candidateKey);
        const shouldReplace = !current
          || priority > current.priority
          || (priority === current.priority && numericValue !== 0 && current.value === 0)
          || (priority === current.priority && eventTime > current.eventTime);

        if (shouldReplace) {
          preferredMappedValues.set(candidateKey, {
            entry,
            targetField,
            value: numericValue,
            priority,
    ...
    preferredMappedValues.forEach(({ entry, targetField, value }) => {
