...
export default function DailyTable({ rows, outlets = [], onEdit, showRupee = false, allOutlets = [] }) {
  ...
  // A midnight/source-sync placeholder is not a confirmed ₹0 collection.
  // Keep it visually distinct until a user or the source system supplies data.
  const isPendingSourceRow = (row) => {
    const isSourceSync = Boolean(row?.autoGenerated || row?.finalSourceSync);
    const hasManualValue = Boolean(row?.addedByPerOutlet && Object.keys(row.addedByPerOutlet).length);
    return isSourceSync && !hasManualValue && outletIds.every((id) => getOutletValue(row, id) === 0);
  };
                ...
                >
                  ...
                  {outletIds.map((outletId, j) => (
                    <td key={String(outletId) + '-' + j} className="whitespace-nowrap px-4 py-3">
                      {isPending ? <span className="text-amber-600">Pending</span> : fmt(getOutletValue(row, outletId))}
                    </td>
                  ))}
                  <td className="whitespace-nowrap px-4 py-3 text-right font-semibold text-orange-600">
                    {isPending ? <span className="text-amber-600">Pending</span> : fmt(rowTotal)}
                  </td>
                  {showEditCol && (
