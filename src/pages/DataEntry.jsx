...
export default function DataEntry() {
  ...
  useEffect(() => {
    ...
    const foundSales = findPreferredRecord(allSalesData, doc =>
    ...
    const foundCash = findPreferredRecord(allCashData, doc =>
      normalizeDate(doc.date || doc.createdAt) === date && doc.outlets && doc.outlets[outlet] !== undefined);
    if (foundCash) { setCash(foundCash.outlets[outlet]); setCashLocked(true); }

    const foundDigital = findPreferredRecord(allDigitalData, doc =>
      normalizeDate(doc.date || doc.createdAt) === date && doc.outlets && doc.outlets[outlet] !== undefined);
    if (foundDigital) { setDigital(foundDigital.outlets[outlet]); setDigitalLocked(true); }

    const foundDamages = findPreferredRecord(allDamagesData, doc =>
    ...
    const foundIncentive = findPreferredRecord(allIncentiveData, doc =>
      normalizeDate(doc.date || doc.createdAt) === date && doc.outlets && doc.outlets[outlet] !== undefined);
    if (foundIncentive) { setIncentive(foundIncentive.outlets[outlet]); setIncentiveLocked(true); }

    const foundAdvance = findPreferredRecord(allAdvanceData, doc =>
      normalizeDate(doc.date || doc.createdAt) === date && doc.outlets && doc.outlets[outlet] !== undefined);
    if (foundAdvance) { setAdvance(foundAdvance.outlets[outlet]); setAdvanceLocked(!foundAdvance.autoGenerated); }
