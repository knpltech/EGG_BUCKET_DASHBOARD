...
const matchesOutletKey = (value, outletKeys) =>
  outletKeys.some((key) => normalizeOutletKey(key) === normalizeOutletKey(value));

const getMappedOutletValue = (values, outletKeys) => {
  if (!values || typeof values !== "object") return { found: false, value: undefined, key: undefined };
  const key = Object.keys(values).find((candidate) => matchesOutletKey(candidate, outletKeys));
  return key === undefined ? { found: false, value: undefined, key: undefined } : { found: true, value: values[key], key };
};

const getRecordTime = (record) => {
  const value = record?.updatedAt || record?.createdAt || record?.addedBy?.timestamp || 0;
  if (value && typeof value.toDate === "function") return value.toDate().getTime();
  const time = new Date(value).getTime();
  return Number.isNaN(time) ? 0 : time;
};

// Data can contain an older automatic/default row alongside a corrected manual
// row. Resolve the value at outlet level, rather than using whichever document
// Firestore returned first.
const findPreferredRecord = (rows, predicate, getPriority = () => 0, getValue = () => undefined) => (rows || [])
  .filter(predicate)
  .sort((left, right) => {
    ...
    if (Boolean(rightValue) !== Boolean(leftValue)) return Number(Boolean(rightValue)) - Number(Boolean(leftValue));
...
export default function DataEntry() {
  ...
  const outletKeys = useMemo(() => {
    const selected = outlets.find((item) => [item?.id, item?.area, item?.name]
  ...
  const { completedDates } = useMemo(() => {
    ...
    const hasMappedEntry = (rows, field, dateKey) => rows.some((doc) =>
      normalizeDate(doc.date || doc.createdAt) === dateKey && getMappedOutletValue(doc[field], outletKeys).found);
    const hasInSales = (d) => hasMappedEntry(allSalesData, "outlets", d);
    const hasInCash = (d) => hasMappedEntry(allCashData, "outlets", d);
    const hasInDigital = (d) => hasMappedEntry(allDigitalData, "outlets", d);
    const hasInDamages = (d) => hasMappedEntry(allDamagesData, "damages", d);
    const hasInNecc = (d) => allNeccData.some((doc) =>
      normalizeDate(doc.date || doc.createdAt) === d && matchesOutletKey(doc.outletId || doc.outlet, outletKeys));
    const hasInIncentive = (d) => hasMappedEntry(allIncentiveData, "outlets", d);
    const hasInAdvance = (d) => hasMappedEntry(allAdvanceData, "outlets", d);
    const hasInFoodAllowance = (d) => hasMappedEntry(allFoodAllowanceData, "outlets", d);

    const allDates = new Set();
    [
      [allSalesData, "outlets"], [allCashData, "outlets"], [allDigitalData, "outlets"],
      [allDamagesData, "damages"], [allNeccData, null], [allIncentiveData, "outlets"],
      [allAdvanceData, "outlets"], [allFoodAllowanceData, "outlets"],
    ].forEach(([rows, field]) => rows.forEach((doc) => {
      const matches = field
        ? getMappedOutletValue(doc[field], outletKeys).found
        : matchesOutletKey(doc.outletId || doc.outlet, outletKeys);
      if (matches) allDates.add(normalizeDate(doc.date || doc.createdAt));
    }));
  ...
  }, [outlet, outletKeys, allSalesData, allCashData, allDigitalData, allDamagesData, allNeccData, allIncentiveData, allAdvanceData, allFoodAllowanceData]);
  ...
  useEffect(() => {
    ...
    setAdvance("0"); setAdvanceLocked(false);
  ...
  useEffect(() => {
    ...
    const getMappedPriority = (doc) => {
      ...
      if (enteredManually) return 4;
      if (!doc.autoGenerated) return 3;
    ...
    const findMappedRecord = (rows, field) => findPreferredRecord(
      ...
      (doc) => {
        ...
        return entry.found && (getMappedPriority(doc) > 1 || Number(entry.value) !== 0);
      },
      getMappedPriority,
    ...
    applyMappedLock(allSalesData, "outlets", setSales, setSalesLocked);
    applyMappedLock(allCashData, "outlets", setCash, setCashLocked);
    applyMappedLock(allDigitalData, "outlets", setDigital, setDigitalLocked);
    applyMappedLock(allDamagesData, "damages", setDamages, setDamagesLocked);
    applyMappedLock(allIncentiveData, "outlets", setIncentive, setIncentiveLocked);
    applyMappedLock(allAdvanceData, "outlets", setAdvance, setAdvanceLocked);
    applyMappedLock(allFoodAllowanceData, "outlets", setFoodAllowance, setFoodAllowanceLocked);
    ...
    // A blank system-created remark is intentionally editable. Lock only a real saved remark.
    const foundRemarks = findPreferredRecord(
      allRemarksData,
      (doc) => {
        if (normalizeDate(doc.date || doc.createdAt) !== date) return false;
        const entry = getMappedOutletValue(doc.remarks || doc.outlets, outletKeys);
        return entry.found && String(entry.value || "").trim() !== "";
      },
      (doc) => getMappedOutletValue(doc.addedByPerOutlet, outletKeys).found ? 4 : !doc.autoGenerated ? 3 : 1,
      (doc) => getMappedOutletValue(doc.remarks || doc.outlets, outletKeys).value,
    );
    if (foundRemarks) {
      const entry = getMappedOutletValue(foundRemarks.remarks || foundRemarks.outlets, outletKeys);
  ...
  useEffect(() => {
    ...
    if (
      prevOutletDateRef.current.outlet === outlet &&
      prevOutletDateRef.current.date === date &&
      prevOutletDateRef.current.summaryRefreshVersion === summaryRefreshVersion
    ) {
      return;
    }
    prevOutletDateRef.current = { outlet, date, summaryRefreshVersion };

    // Use the same outlet matching and manual-record preference as the locking
    // logic. Otherwise an outlet's display name can differ from its stored key,
    // causing fetched summary values to replace a value that was already saved.
    const getMappedPriority = (doc) => {
      const enteredManually = getMappedOutletValue(doc.addedByPerOutlet, outletKeys).found
        || getMappedOutletValue(doc.manuallyEnteredOutlets, outletKeys).found;
      if (enteredManually) return 4;
    ...
    const findSavedMappedRecord = (rows, field) => findPreferredRecord(
      rows,
      (doc) => {
        if (normalizeDate(doc.date || doc.createdAt) !== date) return false;
        const entry = getMappedOutletValue(doc[field], outletKeys);
    ...
    const findSavedNeccRecord = () => findPreferredRecord(
      allNeccData,
      (doc) => normalizeDate(doc.date || doc.createdAt) === date && matchesOutletKey(doc.outletId || doc.outlet, outletKeys),
      (doc) => doc.addedBy ? 4 : !doc.autoGenerated ? 3 : doc.finalSourceSync ? 2 : 1,
      (doc) => doc.rateValue ?? doc.rate,
    );
    const hasSavedMappedRecord = (rows, field) =>
      !isManualReentry && Boolean(findSavedMappedRecord(rows, field));

    const hasSales = hasSavedMappedRecord(allSalesData, "outlets");
    const hasCash = hasSavedMappedRecord(allCashData, "outlets");
    const hasDigital = hasSavedMappedRecord(allDigitalData, "outlets");
    const hasDamages = hasSavedMappedRecord(allDamagesData, "damages");
    const hasNecc = !isManualReentry && Boolean(findSavedNeccRecord());
    const hasIncentive = hasSavedMappedRecord(allIncentiveData, "outlets");
    const hasFoodAllowance = hasSavedMappedRecord(allFoodAllowanceData, "outlets");

    const savedSales = findSavedMappedRecord(allSalesData, "outlets");
    const savedNecc = findSavedNeccRecord();

    if (savedSales && savedNecc) {
      const savedRate = Number(savedNecc.rateValue ?? savedNecc.rate) || 0;
      const savedSalesValue = getMappedOutletValue(savedSales.outlets, outletKeys).value;
      setAiTotalAmount((Number(savedSalesValue) || 0) * savedRate);
      return;
    }
    ...
    const fetchSummary = async () => {
      ...
      try {
        ...
        if (isMounted) {
          ...
          if (!isManualReentry && !hasDigital && data.digitalPayment !== undefined) {
          ...
          if (!hasDamages || !hasIncentive || !hasFoodAllowance) {
            if (!hasDamages) {
  ...
  }, [outlet, date, outletKeys, isLoadingData, allSalesData, allCashData, allDigitalData, allDamagesData, allNeccData, allIncentiveData, allFoodAllowanceData, isManualReentry, summaryRefreshVersion, todayIso]);
  ...
  const handleDeleteDate = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API}/data-entry/reset/date/${date}/outlet/${encoded}`, {
        method: "DELETE",
        headers,
      });
      if (!response.ok) {
        const body = await response.text().catch(() => "");
        throw new Error(`Delete failed with status ${response.status}${body ? `: ${body}` : ""}`);
      }
      await response.json();
          ...
          <>
            ...
            {allAlreadyLocked && (
              <div className="mb-4 rounded-xl bg-green-50 border border-green-200 px-4 py-3 flex items-center gap-2">
                ...
                <div>
                  <p className="text-sm font-semibold text-green-700">Saved data loaded</p>
                  <p className="text-xs text-green-600">Saved data loaded. Required fields for this outlet and date are locked.</p>
                </div>
              </div>
            )}
