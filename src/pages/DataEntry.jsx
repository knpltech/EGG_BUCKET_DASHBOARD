...
const findPreferredRecord = (rows, predicate) => (rows || [])
  .filter(predicate)
  .sort((left, right) => Number(Boolean(left?.autoGenerated)) - Number(Boolean(right?.autoGenerated)))[0];
...
export default function DataEntry() {
  ...
  useEffect(() => {
    ...
    const foundNecc = findPreferredRecord(allNeccData, (doc) =>
      normalizeDate(doc.date || doc.createdAt) === date && matchesOutletKey(doc.outletId || doc.outlet, outletKeys));
