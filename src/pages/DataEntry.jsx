...
export default function DataEntry() {
  ...
  const loadAllData = useCallback(async () => {
    ...
    try {
      ...
      if (requestId === loadRequestRef.current) setIsLoadingData(false);
    }
  }, [date, todayIso]);
  ...
  useEffect(() => {
    ...
    if (foundNecc) {
      setNeccrate(formatNeccRate(foundNecc.rateValue));
      setNeccrateLocked(true);
      savedLocks.necc = true;
      if (savedSalesRecord) {
        const savedSalesValue = getMappedOutletValue(savedSalesRecord.outlets, outletKeys).value;
        setAiTotalAmount((Number(savedSalesValue) || 0) * (Number(foundNecc.rateValue ?? foundNecc.rate) || 0));
      }
    }
    savedLocksRef.current = savedLocks;
  ...
  useEffect(() => {
    ...
    // Kept as local flags for diagnostics; the ref avoids delaying the fast
    // summary request while history is loading.
    const hasSales = Boolean(savedLocksRef.current.sales);
    const hasCash = Boolean(savedLocksRef.current.cash);
    const hasDigital = Boolean(savedLocksRef.current.digital);
    ...
    const fetchSummary = async () => {
      ...
      try {
        ...
        if (isMounted) {
          ...
          if (!savedLocksRef.current.sales && data.salesQty !== undefined) {
            console.log("🟢 CALLING setSales with:", data.salesQty);
            setSales(String(data.salesQty));
          }
          if (!isManualReentry && !savedLocksRef.current.digital && data.digitalPayment !== undefined) {
          ...
          if (!isManualReentry && !savedLocksRef.current.cash) {
  ...
  }, [outlet, date, isManualReentry, summaryRefreshVersion]);
  ...
  const handleDeleteDate = async () => {
    ...
    try {
      ...
      // Keep the form responsive after saving; the background refresh verifies
      // the saved data without delaying the confirmed state on screen.
      void loadAllData();
      prevOutletDateRef.current = { outlet: "", date: "" };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // A summary request may still be in flight. Mark these values as saved
      // immediately so that late response cannot replace the confirmed entry.
      savedLocksRef.current = {
        ...savedLocksRef.current,
        necc: Boolean(neccrateLocked || neccrate !== ""),
        sales: Boolean(salesLocked || sales !== ""),
        damages: Boolean(damagesLocked || damages !== ""),
        cash: Boolean(cashLocked || cash !== ""),
        digital: Boolean(digitalLocked || digital !== ""),
        incentive: Boolean(incentiveLocked || incentive !== ""),
        advance: Boolean(advanceLocked || advance !== ""),
        foodAllowance: Boolean(foodAllowanceLocked || foodAllowance !== ""),
      };
