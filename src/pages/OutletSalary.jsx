...
export default function OutletSalary() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [outletResponse, salaryResponse] = await Promise.all([
  ...
  const openEditModal = (month) => {
    ...
    sortedOutlets.forEach((outlet) => {
      const amount = toNumber(existing?.outlets?.[outlet.id]);
            ...
            <tbody>
              ...
              {tableRows.map((row) => (
                <tr key={`${row.year}-${row.month}`} className="border-t border-gray-100">
                  <td className="px-4 py-3 font-medium text-gray-800">{monthLabel(Number(row.month)) || row.monthName}</td>
                  {sortedOutlets.map((outlet) => (
                    <td key={outlet.id} className="px-4 py-3 text-right text-gray-700">
