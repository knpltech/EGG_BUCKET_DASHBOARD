...
const Neccrate = () => {
  ...
  const fetchRates = async () => {
    try {
      ...
      const res = await fetch(`${API_URL}/neccrate/all${params.size ? `?${params}` : ""}`, { headers });
      if (!res.ok) throw new Error("Failed to load NECC rates");
  ...
  const { pivotMap, sortedDates } = useMemo(() => {
    const nextPivotMap = {};
    const setRate = (date, key, value) => {
      const existing = nextPivotMap[date][key];
      const shouldReplace = !existing
        || (existing.autoGenerated && !value.autoGenerated)
        || ((Number(existing.rate) || 0) === 0 && (Number(value.rate) || 0) !== 0);
      if (shouldReplace) nextPivotMap[date][key] = value;
    };
