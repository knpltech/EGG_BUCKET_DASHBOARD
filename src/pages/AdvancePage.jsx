...
const AdvancePage = () => {
  ...
  const editTotal = useMemo(
    () => Object.values(editValues).reduce((sum, value) => sum + (Number(value) || 0), 0),
    [editValues]
  );
  ...
  const handleEditSave = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/advance/${editRow.id}`, {
        method: "PATCH",
        headers: {
      ...
      setRows((prev) =>
        prev.map((r) =>
          r.id === editRow.id ? { ...r, outlets: numericOutlets, total } : r
        )
      );
        ...
        {isAdmin && !isReadOnly && editModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-30 p-4">
            <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6 w-full max-w-md max-h-[80vh] overflow-y-auto">
              <h2 className="font-semibold mb-4 text-lg">Edit Advance ({editRow.date})</h2>
