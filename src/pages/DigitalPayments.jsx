...
export default function DigitalPayments() {
  ...
  const isPendingSourceRow = useCallback((row) => (
    Boolean(row?.autoGenerated || row?.finalSourceSync)
    && !Object.keys(row?.addedByPerOutlet || {}).length
    && displayedOutlets.every((outlet) => getOutletValue(row?.outlets, outlet) === 0)
  ), [displayedOutlets]);
                  ...
                  {filteredRows.length === 0 ? (
                    ...
                    <>
                      {filteredRows.map((row, idx) => {
                        ...
                        const isPending = isPendingSourceRow(row);
                        return (
                          <tr key={row.id} className={`text-xs text-gray-700 md:text-sm ${idx % 2 === 0 ? "bg-white" : "bg-gray-50/60"}`}>
                              ...
                              return (
                                <td key={outlet.id} className="whitespace-nowrap px-4 py-3 align-top">
                                  <div className="relative inline-block" data-audit-cell>
                                    ...
                                    >
                                      <span className="block font-medium text-gray-900">{isPending ? "Pending" : formatSmart(getOutletValue(row.outlets, outlet))}</span>
                            ...
                            <td className="sticky right-0 bg-inherit z-10 whitespace-nowrap px-4 py-3 text-right font-semibold">
                              {isPending ? "Pending" : formatSmart(rowTotal)}
                            </td>
                          </tr>
                        );
                      })}
