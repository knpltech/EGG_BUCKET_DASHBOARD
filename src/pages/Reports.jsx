...
const Reports = () => {
  ...
  const quantityTrendOutlets = useMemo(() => {
    ...
    // Sales records may use an outlet ID, area, or name as their key. Keep
    // every alias so historical and current entries are read consistently.
  ...
  useEffect(() => {
    ...
    const loadQuantityTrend = async () => {
      ...
      try {
        ...
        // Mirror the Daily Sales table: a manual value must not be replaced
        // by a duplicate automatic placeholder row for the same date.
        const rows = mergeDailyRecords(rawRows);
        ...
        const outletValueForRow = (row, outlet) => {
          ...
          // Prefer a non-zero matching alias. If all matched aliases are
          // zero, preserve that as a genuine zero sale.
          for (const alias of outlet.salesKeys || []) {
            if (values[alias] === undefined) continue;
            const value = Number(values[alias]) || 0;
            if (value !== 0) return value;
            fallbackValue = value;
          }

          // Some legacy entries only differ in capitalization or whitespace
          // (for example, "BANDEPALYA" vs "Bandepalya"). Match those too so
          // every outlet follows the same charting rule.
          const normalizedAliases = new Set((outlet.salesKeys || []).map((alias) =>
            String(alias).trim().toLowerCase().replace(/\s+/g, ' ')
          ));
          for (const [storedKey, rawValue] of Object.entries(values)) {
            const normalizedKey = String(storedKey).trim().toLowerCase().replace(/\s+/g, ' ');
        ...
        while (cursor <= endDate) {
          ...
          quantityTrendOutlets.forEach((outlet) => {
            // Missing and automatic-pending records are unknown, rather than
            // zero sales; null renders as a gap in the trend line.
            const isPending = sourceRow?.autoGenerated
              && !sourceRow?.finalSourceSync
              && !Object.keys(sourceRow?.addedByPerOutlet || {}).length;
            point[outlet.key] = sourceRow && !isPending ? outletValueForRow(sourceRow, outlet) : null;
          });
