...
export default function DailyDamages() {
      ...
      try {
        ...
        if (Array.isArray(data)) {
          setDamages(mergeDailyRecords(data.map(d => ({
            id: d.id, date: d.date,
            damages: (d.damages && typeof d.damages === 'object') ? d.damages : {},
            total: d.total || 0,
            autoGenerated: d.autoGenerated,
            finalSourceSync: d.finalSourceSync,
            addedByPerOutlet: d.addedByPerOutlet,
          })), "damages").map((row) => ({
            id: row.id,
            date: row.date,
            ...(row.damages || {}),
            total: Object.values(row.damages || {}).reduce((sum, value) => sum + (Number(value) || 0), 0),
            autoGenerated: row.autoGenerated,
            finalSourceSync: row.finalSourceSync,
            addedByPerOutlet: row.addedByPerOutlet,
          })));
        }
      } catch (err) {}
    };
  ...
  const isPendingSourceRow = (row) => (
    Boolean(row?.autoGenerated || row?.finalSourceSync)
    && !Object.keys(row?.addedByPerOutlet || {}).length
    && displayedOutletObjects.every((outlet) => getOutletValue(row, outlet) === 0)
  );
              ...
              <table className="w-full border-collapse min-w-max">
                ...
                <tbody>
                  {filteredData.map((d, i) => {
                    ...
                    const isPending = isPendingSourceRow(d);
                    return (
                      <tr key={i} className="border-t text-sm hover:bg-gray-50 transition">
                        ...
                        {displayedOutlets.map((outletId) => {
                          ...
                          return (
                            <td key={outletId} className="p-3 text-center">{isPending ? <span className="text-amber-600">Pending</span> : Math.round(getOutletValue(d, outlet || outletId))}</td>
                          );
                        })}
                        <td className="p-3 text-center font-bold text-orange-600 sticky right-0 bg-white z-10">
                          {isPending ? <span className="text-amber-600">Pending</span> : rowTotal}
                        </td>
                      </tr>
                    );
                  })}
